...
CREATE OR REPLACE PROCEDURE proc.update_serviceinstance(
...
BEGIN
    ...
    SET updated_by = p_actor_name,
        ...
        consulted_id = v_consulted_id,
        informed_id = v_informed_id,
        data = v_data
    WHERE id = v_id;
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated. serviceinstance ID % not found', v_id ;
    END IF;

           p_auditlog_params := jsonb_build_object(
        'tenant', '',
        'searchindex', '',
        'name', 'update_serviceinstance',
        'status', 'success',
        'description', '',
        'action', 'update_serviceinstance',
